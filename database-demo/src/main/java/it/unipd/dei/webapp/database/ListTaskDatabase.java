...
public final class ListTaskDatabase {
	 ...
	 */
	private static final String STATEMENT = "WITH RECURSIVE proj_subpart AS (SELECT Parent, Child, ProjectID, 1 AS Depth FROM Task as t INNER JOIN Compose AS C  ON t.taskID = C.Child WHERE isRoot = TRUE UNION ALL SELECT C.Parent, C.Child, C.ProjectID, Depth + 1 FROM proj_subpart AS pr INNER JOIN Compose AS C ON C.Parent = pr.Child) SELECT p.ProjectID AS Project_Name, t.taskID, tmt.TemplateID AS Task_Description, Depth, tmt.Description AS General_task_description FROM Project AS p INNER JOIN proj_subpart AS ps ON p.ProjectID = ps.ProjectID INNER JOIN Task AS t ON ps.Child = t.taskID INNER JOIN Template AS tmt ON t.TemplateID = tmt.TemplateID WHERE p.ProjectID = ? AND depth < 3 ORDER BY p.ProjectID, Depth ASC";
		...
		try {
			...
			while (rs.next()) {
				tasks.add(new Task( 
						...
						UUID.fromString(rs.getString("t.taskID")),
